...
class Database(DatabaseConfiguration):
    ...
    async def get_all_products(self):
        query = await base_db.client.product_collection.find()
        product_list = []
    ...
    async def get_product(self, product_id):
        query = await base_db.client.product_collection.find_one({"_id": ObjectId(product_id)})
    ...
    async def update_product(self, product_id, product: Product):
        query  = await base_db.client.product_collection.update_one({"_id": ObjectId(product_id)},{"$set": product.dict()})
    ...
    async def partial_update_product(self, product_id, product: UpdateProduct):
        ...
        query  = await base_db.client.product_collection.update_one({"_id": ObjectId(product_id)},{"$set": product_update})
        return query.modified_count

    @start_db
    async def delete_product(self, product_id):
        query = await base_db.client.product_collection.delete_one({"_id": ObjectId(product_id)})
